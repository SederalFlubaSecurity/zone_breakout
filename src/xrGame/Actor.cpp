...
CActor::CActor() : CEntityAlive(),current_ik_cam_shift(0)
{
	...
	m_wounded_idle.invalidate();
	m_wounded_walk_fwd.invalidate();
	m_wounded_walk_back.invalidate();
	m_wounded_walk_l.invalidate();
	m_wounded_walk_r.invalidate();
...
//.	m_vehicle_anims			= xr_new<SActorVehicleAnims>();
	...
	m_wounded_state			= false;
	m_wounded_force_camera	= false;
	m_wounded_saved_camera	= eacFirstEye;
...
void CActor::UpdateWoundedState()
{
	if (!g_Alive())
	{
		m_wounded_state = false;
	...
	const bool is_wounded = actor_conditions.IsWounded();
	if (is_wounded == m_wounded_state)
	{
		if (is_wounded && m_wounded_force_camera && cam_active != eacLookAt)
			cam_Set(eacLookAt);
		return;
	}

	m_wounded_state = is_wounded;
	if (is_wounded)
	{
		m_wounded_saved_camera = cam_active;
		if (cam_active != eacLookAt)
		{
			cam_Set(eacLookAt);
			m_wounded_force_camera = true;
		}
		else
		{
			m_wounded_force_camera = false;
	...
	if (m_wounded_force_camera && cam_active == eacLookAt)
		cam_Set(m_wounded_saved_camera);
	m_wounded_force_camera = false;
}
